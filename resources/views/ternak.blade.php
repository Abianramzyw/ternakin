...
@section('container')
    <div class="container">
        <div class="row justify-content-center mb-5">
            <div class="col-md-8">
                <h1 class="mb-3">Nomer Ternak : {{ $ternaks->id }}</h1>
                <p>Nama Peternak <a href="/dataternak?user={{ $ternaks->user->nama_akun }}" class="text-decoration-none">
                        {{ $ternaks->user->nama_akun }} </a> </a> Dari Jenis {{ $ternaks->jenisternak->nama_jenis_ternak }}
                    {{-- <a
                ...
                <h5>Status Terjual : {{ $ternaks->statusterjual->nama_status_terjual }}</h5>
                <h5>Foto :</h5>
                @if ($ternaks->image)
                    <img src="{{ asset('storage/' . $ternaks->image) }}" alt="{{ $ternaks->jenisternak->nama_jenis_ternak }}"
